...
public :
  ...
  std::map<std::pair<int,int>, vector<double>> myMap;
  ...
  int gen_ntaus, gen_ntaup, gen_ntaum;
  vector<double>  *signalParameters;
   ...
   etau_analyzer(const char* file1, const char* file2, string isMC, string SampleName, string year, const char* signal_para);
   ~etau_analyzer();
...
etau_analyzer::etau_analyzer(const char* file1, const char* file2, string isMC, string sampleName, string year, const char* signal_para)
{
...
void etau_analyzer::make_met_corr_plot()
{
  TLorentzVector raw_met, corrected_met ;
  raw_met.SetPtEtaPhiE(pfMET ,0,pfMETPhi,pfMET);
  corrected_met = MetRecoilCorrections(EleIndex, TauIndex, corrected_met);
  
  double correction = abs(corrected_met.Pt()-raw_met.Pt())/raw_met.Pt();
  ...
  int num_jets = min(my_njets, 2);
  double Zpt = BosonP4.Pt();
  
  int jet_bin = num_jets + 1;
  int Zpt_bin = 0;

  if(Zpt < 10) Zpt_bin = 1;
  else if(Zpt <= 10) Zpt_bin = 1;
  else if(Zpt > 10 && Zpt <=20) Zpt_bin = 2;
  else if(Zpt > 20 && Zpt <=30) Zpt_bin = 3;
  else if(Zpt > 30 && Zpt <=40) Zpt_bin = 4;
  else if(Zpt > 40 && Zpt <=50) Zpt_bin = 5;
  else if(Zpt > 50 ) Zpt_bin = 6;
  
  //std::map<std::pair<int,int>, vector<double>> myMap;
  myMap[std::make_pair(jet_bin,Zpt_bin)].push_back(correction);

  // h_met_corr->SetBinContent(num_jets+1, Zpt_bin, correction );
}
