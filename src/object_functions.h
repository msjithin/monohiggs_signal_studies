   ...
   for(int iMu=0;iMu<nMu;iMu++)
     {
       ...
       if( relMuIso < 0.15 && muPt->at(iMu)<100) 
	 muonIso = true;
       else if (muPt->at(iMu)>=100)
	 muonIso = true;
...
std::vector<int> mutau_analyzer::getTauCand_med(double tauPtCut, double tauEtaCut, int shift){
  std::vector<int> tmpCand;
  tmpCand.clear();
  TLorentzVector dau2;
  //Loop over taus      
...
std::vector<int> mutau_analyzer::getTauCand_vl(double tauPtCut, double tauEtaCut, int shift){
  std::vector<int> tmpCand;
  tmpCand.clear();
  TLorentzVector dau2;
  //Loop over taus      
